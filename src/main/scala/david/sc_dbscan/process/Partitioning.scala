...
object Partitioning {
  ...
  //  ----------------------------------------------------------
  def getInitalPartition(files: RDD[String], eps: Double, coefficient: Boolean, order: Map[Int, Int], schema: Map[Int, Set[Triple]]): RDD[(Set[Int], Array[Noeud])] = {
    ...
    val noeudsList: RDD[(Set[Int], Array[Noeud])] = files.flatMap {
      line =>
        ...
        if (!line.equals("") && !line.equals(" +")) {
          var noeud = Saturation.saturateEntities(NodeBuilder.createNode(line, coefficient), schema)
  ...
  //  --------------------------------------------------------------
  def getFinalPartitions(initialPartition: RDD[(Set[Int], Array[Noeud])], nodeCapacity: Int, eps: Double, order: Map[Int, Int], coefficient: Boolean): RDD[(Set[Int], Array[Noeud])] = {
    ...
    var finalPartitions = initialPartition.flatMap { f =>
      ...
      var result: Array[Partition] = Partitioning.getSmallerPartitions(p, nodeCapacity, eps, order, coefficient)
...
//        finalPartitions.map( f => (f._1+" "+f._2.toList.toString()) ).saveAsTextFile("partitions")
  ...
  //  ----------------------------------------------------------
  def getSmallerPartitions(initialPartition: Partition, nodeCapacity: Int, eps: Double, order: Map[Int, Int], coefficient: Boolean): Array[Partition] = {

    var finalPartitions: Array[Partition] = Array()
    val initialPartitionSize = initialPartition.getListNoeud().length

    if (initialPartitionSize > 1 || coefficient) {
      if (initialPartitionSize <= nodeCapacity) {
        ...
        newPartitions.foreach { f =>
          finalPartitions = finalPartitions ++ getSmallerPartitions(f, nodeCapacity, eps, order, coefficient)
        }
      }
    }
